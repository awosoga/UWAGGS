...
approaches can be used to extract data. We will cover some
...
this case, then we simply use `rvest::html_table`, which will return a dataframe or list
of data frames.
...
scrape_data <- function(url, css_selection) {
  return(url %>% read_html() %>% 
           html_elements(css_selection) %>% html_text())
}
